...
def _portfolio_example() -> dict[str, Any]:
    ...
    return {
        ...
        "exposure": {
            ...
            "largest_position_weight": "0.0800",
            "position_count": 1,
        },
        "provenance": {
...
def test_design_trade_signal_example_validates() -> None:
    signal = parse_trade_signal(trade_signal_example())
...
@pytest.mark.unit
def test_rejects_expired_or_inverted_signal_window() -> None:
    payload = trade_signal_example()
    ...
    signal = parse_trade_signal(trade_signal_example())
    assert signal.is_expired(datetime(2026, 7, 24, 19, 0, 10, tzinfo=UTC))
    with pytest.raises(ValueError, match="expired"):
        signal.require_active(datetime(2026, 7, 24, 19, 0, 10, tzinfo=UTC))
    signal.require_active(datetime(2026, 7, 24, 18, 45, 0, tzinfo=UTC))
...
def test_hold_cannot_become_order_or_carry_target_weight() -> None:
    ...
    payload["strength"] = "0"
    payload["reason_codes"] = ["NO_EDGE"]
    signal = parse_trade_signal(payload)
    assert not signal.may_become_order()


@pytest.mark.unit
def test_portfolio_snapshot_rejects_inconsistent_exposure_and_future_orders() -> None:
...
def test_context_and_signal_round_trip() -> None:
    ...
    signal = parse_trade_signal(deepcopy(trade_signal_example()))
