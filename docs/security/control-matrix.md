...
| `T-016` | `critical` | `robinhood_read_and_live` | Read Broker owner | `partial` | `partial` | `E-SECRET-ROLES`, `E-YAHOO-LIVE-DENY`, `E-HEALTH-NOTREADY`, `P-MCP-ALLOWLIST`, `P-GATEWAY-PROSE` | The external rh-mcp v0.3.0 release is independently reviewed; the stable P06-T0 ainvest adapter boundary was independently reviewed in #104/#105/#107, while the narrow v0.3 pin refresh awaits its own review. P06-T0 pins and verifies the release, artifact and manifests; imports only the published gateway surface; exposes the same named 10-operation projection over the 35 mutates=false capabilities; rejects the 11 approved non-trading mutations and all 8 denied trading capabilities; sanitizes failures and SDK-neutral envelopes; and discards provider-controlled prose at the adapter boundary with deterministic fixtures. T-016 remains partial, not passing or complete: P06-T1 normalization, P06-T2 sinks and deployment evidence, real owner-assisted authorization, and P-GATEWAY-PROSE end-to-end evidence are still pending. |
<!-- control-matrix:end -->
...
| External `rh-mcp` evidence | partial | `rh-mcp` `v0.3.0` is independently reviewed (`APPROVED_FOR_AINVEST_INTEGRATION`, 2026-08-12, bound to tag commit `078b1e6`) and its tag, artifact SHA-256 digests, build provenance, manifest version, expected full-manifest digest, and envelope version are pinned in `docs/tasks/status.md`; the separate ainvest P06-T0 adapter, read projection, sanitization, and deterministic fixtures are implemented and independently reviewed in #104, #105, and #107, with the narrow v0.3 refresh awaiting its own independent review | The external approval applies only to that exact artifact, while the original ainvest review applies to the stable P06-T0 boundary and the refresh still requires independent review. T-016 remains partial pending P06-T1 normalization, P06-T2 sinks/deployment, real authorization, and end-to-end P-GATEWAY-PROSE evidence. |
