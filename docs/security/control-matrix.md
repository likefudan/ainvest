...
| `T-016` | `critical` | `robinhood_read_and_live` | Read Broker owner | `partial` | `partial` | `E-SECRET-ROLES`, `E-YAHOO-LIVE-DENY`, `E-HEALTH-NOTREADY`, `P-MCP-ALLOWLIST`, `P-GATEWAY-PROSE` | The exact external rh-mcp v0.4.2 release artifacts and stable P06-T0 ainvest adapter boundary are independently audited; the executable pin implementation remains subject to exact-head review before merge. P06-T0 pins and verifies the release, artifact and manifests; imports only the published gateway surface; explicitly keeps allow_mutations false; exposes the same named 10-operation projection over the 36 allowed non-mutating capabilities; rejects the 11 approved non-trading mutations, all 8 denied trading capabilities, and all 4 denied non-mutating SEC capabilities; sanitizes failures and SDK-neutral envelopes; and discards provider-controlled prose at the adapter boundary with deterministic fixtures. P06-T1 normalization and the P06-T2 Part 1 display sink are merged. T-016 remains partial, not passing or complete: P06-T2 Part 2 still requires trustworthy canonical instrument identity, verified Agentic-account binding, and regular-session evidence; deployment evidence, owner-assisted real status/read validation against v0.4.2, and P-GATEWAY-PROSE end-to-end evidence also remain pending. |
<!-- control-matrix:end -->
...
| External `rh-mcp` evidence | partial | `rh-mcp` `v0.4.2` release assets are bound to annotated tag object `a866c84d…` and commit `36cd95c`, their SHA-256 values and GitHub attestations are verified, and the manifest's 36/11/8/4 split is pinned in `docs/tasks/status.md`; the ainvest P06-T0 adapter, ten-read projection, sanitization, and deterministic fixtures remain deliberately narrow; P06-T1 normalization and P06-T2 Part 1 display are merged through #111/#114/#117 | The artifact evidence applies only to the exact v0.4.2 assets. The pin implementation still requires exact-head independent review and green merge gates. T-016 remains partial pending P06-T2 Part 2's trustworthy canonical identity, verified Agentic-account binding, and regular-session evidence; deployment evidence, owner-assisted real status/read validation against v0.4.2, and end-to-end P-GATEWAY-PROSE evidence also remain pending. |
