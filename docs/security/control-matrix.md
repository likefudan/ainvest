...
| External `rh-mcp` evidence | partial | `rh-mcp` `v0.3.0` is independently reviewed (`APPROVED_FOR_AINVEST_INTEGRATION`, 2026-08-12, bound to tag commit `078b1e6`) and its tag, artifact SHA-256 digests, build provenance, manifest version, expected full-manifest digest, and envelope version are pinned in `docs/tasks/status.md`; the ainvest P06-T0 adapter, read projection, sanitization, deterministic fixtures, and narrow v0.3 refresh are implemented and independently reviewed through #104/#105/#107/#127; P06-T1 normalization and P06-T2 Part 1 display are merged through #111/#114/#117 | The external approval applies only to that exact artifact. T-016 remains partial pending P06-T2 Part 2's trustworthy canonical identity, verified Agentic-account binding, and regular-session evidence; deployment evidence, owner-assisted real status/read validation, and end-to-end P-GATEWAY-PROSE evidence also remain pending. |
