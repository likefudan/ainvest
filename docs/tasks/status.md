...
[#127](https://github.com/likefudan/ainvest/pull/127). The v0.3.3 claim and
independently reviewed implementation squash-merged in
[#142](https://github.com/likefudan/ainvest/pull/142) as
`b72104232947472346dd7978b8652672125a3dca` and
[#143](https://github.com/likefudan/ainvest/pull/143) as
`8fc77f008041ea9ea130c0d5920ddf26f7b98a6d`. Owner-assisted validation on
2026-08-26 proved v0.3.3 `auth-status` healthy and `status` ready against the
exact pin; Telegram status, account eligibility, and a representative quote
succeeded. Sanitized `result_too_large` for history and
`account_secret_missing` for portfolio are claimed below as two narrow
follow-ups; no account data is recorded here.
`P05-T10` claim and implementation squash-merged in
[#135](https://github.com/likefudan/ainvest/pull/135) and
[#136](https://github.com/likefudan/ainvest/pull/136). The `P05-T9` claim and
implementation then squash-merged in [#138](https://github.com/likefudan/ainvest/pull/138)
and [#139](https://github.com/likefudan/ainvest/pull/139), after the #140 pip
security prerequisite and a final post-rebase review. Later candidates
...
- **Status/owner:** `merged` / `completed` —
  ...
  P05-T10 and P05-T9 are also merged.
...
- **Handoff and live validation:** the implementation completed latest-main
  ...
  checks, and squash merge. P05-T10 provisioning and P05-T9 Telegram
  display-only reads are also merged. Owner-assisted `rh-mcp v0.3.0 status`
  reports ready; individual real reads, real Bot validation, and end-to-end
  Telegram reads remain owner-assisted evidence and must not put credentials
  or account data in Git, PRs, logs, or chat.
...
- **Next work:** P05-T10 Telegram Bot-environment provisioning and P05-T9
  ...
  the completed P06-T0 `v0.3.3` maintenance record follows immediately below.
  The slice remains
...
##### Completion envelope: P06-T0 `rh-mcp` `v0.3.3` pin refresh

- **Status/owner:** `merged` / `completed` —
  `p06_t0_rh_mcp_v033_pin_refresh`. Claim
  ...
  [#143](https://github.com/likefudan/ainvest/pull/143) independently reviewed
  and squash-merged as `8fc77f008041ea9ea130c0d5920ddf26f7b98a6d`.
- **Historical branch/worktree/base:** `agent/rh-mcp-v033-pin` /
  `.worktrees/rh-mcp-v033-pin`, based on exact claim squash
  `b72104232947472346dd7978b8652672125a3dca`.
- **Task identity and merge order:** this was narrow maintenance under existing
  `P06-T0`, not a new task ID and not a reopening of P06-T1, P06-T2, P05-T9,
  or P05-T10. The claim merged first; the implementation then started from the
  claim squash, obtained independent functionality/readability review of exact
  head `519610eed54f63c4a271f203b75c5b3467bd5aa2`, and squash-merged serially.
- **Review and remediation chain:** the exact released-artifact
  [audit](https://github.com/likefudan/ainvest/pull/142#issuecomment-5419106779)
  approved the pin with the two controlled P2 residuals recorded below. The
  independent claim review then posted two P2
  [findings](https://github.com/likefudan/ainvest/pull/142#issuecomment-5419133948),
  exact-head [remediation](https://github.com/likefudan/ainvest/pull/142#issuecomment-5419183622)
  resolved both, and independent
  [approval](https://github.com/likefudan/ainvest/pull/142#issuecomment-5422801658)
  confirmed no material P0-P2 finding remained before #142 merged. The
...
- **Exact released-artifact audit:** a separate audit subagent's
  ...
  claim review and implementation review permitted no unresolved P0-P3 finding
  at merge. The two documented P2 residuals above count as
...
- **Current release identity:** annotated tag object
...
- **Current artifact identity:** wheel
...
- **Current manifest identity:** format `1.2`, canonicalization
...
- **Implementation handoff:** the official wheel and sdist were re-downloaded
  and matched the recorded hashes and sizes. The committed manifest is
  byte-identical to the wheel member; all eleven payload fixtures moved with
  no content edit. The lock diff changes only the root broker reference and
  direct `rh-mcp` entry from 0.3.0 to 0.3.3; no `mcp`, `httpx2`, or transitive
  package moved. Focused Robinhood/dependency/display/Telegram tests passed
  520/520; the independent review expanded that focused verification to
  566/566. `./scripts/dev verify` passed: mypy was clean across 259 source
  files; unit 1,523 passed with one documented optional-runtime skip; contract
  204 passed; integration 53 passed; aggregate 1,780 passed with the same skip
  and 87.30% branch coverage. The full locked dependency audit found no known
  vulnerability; its expected URL-requirement line cannot map `rh-mcp` to an
...
| Tag | `v0.3.3` (annotated tag object `617b6db3b740bb69c4b93e2e92c17e2fc87a4b03`) |
| Tagged commit | `68830c28db67632044f078c78b364622d9b66e16` |
| Release | <https://github.com/likefudan/rh-mcp/releases/tag/v0.3.3>, published `2026-08-25T23:38:10Z`, not a draft and not a prerelease |
...
|---|---|---:|
| `rh_mcp-0.3.3-py3-none-any.whl` | `b32eea3844cb1f4a80404f969eca5102ec727d1900227c199d03817463124040` | `208920` |
| `rh_mcp-0.3.3.tar.gz` | `c61ff3365279825a4ca7c39dd1b9ba0039db69a66c653c3c10c32e8d5a8247b4` | `483685` |

The public assets, `SHA256SUMS`, independently reproducible wheel and sdist,
and GitHub/Sigstore attestations bind these bytes to `refs/tags/v0.3.3`, the
tagged commit above, and `.github/workflows/release.yml`. The artifact audit
returned `APPROVED_FOR_AINVEST_PIN_WITH_CONDITIONS` with no P0/P1 finding.
...
| `provider_surface_digest` | `sha256:3e3f1d3e3e63bef64a2270d9cd238e12c78b247e82c4e717fa3302b0e9e970f8` |
| **`expected_manifest_digest`** | **`sha256:df71febf46c1e594da56f7e0205357af091a5b1fc7726bdf05259cd53f289bdc`** |
| Result envelope version | `1.0` |

The artifact remains exactly **54 entries: 35 allowed reads with
`mutates=false`, 11 allowed non-trading mutations with `mutates=true`, and 8
denied trading/simulation capabilities**. No name, provider-tool binding,
disposition, or `mutates` value moved from v0.3.0, and ainvest retains its exact
10-member named read projection. All eleven P06-T1 payload fixtures validate
unchanged against the shipped schemas. Provider-controlled guide,
description, annotation, and rationale text remains discarded before model,
Telegram, CLI, or log context.

The owner accepted two exact-artifact P2 residuals: release-governance evidence
is weaker than the artifact/provenance evidence, and future manifest refreshes
must manually review nested mutation constraints and the reads' top-level
property sets. This acceptance does not cover any future source, artifact,
manifest, provider drift, or a violation of the pin, projection, prose-discard,
or owner-assisted validation controls.

##### Historical external dependency pin: `likefudan/rh-mcp` `v0.3.0`

This subsection preserves the previous executable authority and its independent
review evidence. It is not an executable or future implementation authority
after the v0.3.3 implementation is squash-merged. The `v0.2.0` subsection
below is also historical evidence only.
...
[initial independent findings](https://github.com/likefudan/ainvest/pull/145#issuecomment-5426714408),
[remediation](https://github.com/likefudan/ainvest/pull/145#issuecomment-5426797032),
and [final approval](https://github.com/likefudan/ainvest/pull/145#issuecomment-5431909623)
before squash `fa5e30f356a4082eb7a8b88c7d90ec7b3cbd68b1`. P05-T11
implementation [#147](https://github.com/likefudan/ainvest/pull/147) records
the [cancellation-boundary finding](https://github.com/likefudan/ainvest/pull/147#issuecomment-5435586402),
[remediation](https://github.com/likefudan/ainvest/pull/147#issuecomment-5435658038),
and [final approval](https://github.com/likefudan/ainvest/pull/147#issuecomment-5435682285)
before squash `1a724e2861dc811c6c29717f6157c073bcc516f8`. P05-T9 history
maintenance [#146](https://github.com/likefudan/ainvest/pull/146) was
[independently approved](https://github.com/likefudan/ainvest/pull/146#issuecomment-5435792017)
before squash `6815e7e03a3f2609e70d1e5b7281ff630c38e1be`. Its exact-head
verification passed 112 focused tests, 194 history/dependency/architecture
tests, 1,566 unit tests plus one optional-runtime skip, 204 contract tests, 53
...
- **Status/owner:** `merged` — implementation owner
  ...
  below is now merged. P05-T10 provisioning and P05-T9 Telegram display-only
  reads are also merged.
...
- **Owner-assisted validation prerequisite, not offline blocker:** staging Bot
  identity, bound private recipient, `getMe`, private-message delivery, and
  display-only reads are owner-validated. Production still requires its own
  real Bot, positive numeric expected Bot ID, bound numeric private
  `(user_id, private_chat_id)` recipient record, and approved secret storage
  before production integration can be accepted under proposed `DEC-010`.
  These real values are never added to `.env.example`, snapshots, fixtures, or
...
- **Forbidden scope and handoff:** no callback approval decision (`P05-T1`),
  ...
  completed. P05-T10 provisioning/validation and P05-T9 Telegram display-only
  reads are merged.
...
- **Title/status:** Expose Display-Only Robinhood Queries in Telegram —
  `merged` / `completed`; this completion record claims no successor work.
  [#119](https://github.com/likefudan/ainvest/pull/119) assigned the unique task
  ID; it did not claim or implement the task.
- **Claim and implementation:** the docs claim used historical
  `agent/p05-t9-claim` / `.worktrees/p05-t9-claim` from immutable base
  `24d49c7e2817aa7884a3e86abdefc30aefade5cf` and squash-merged in
  [#138](https://github.com/likefudan/ainvest/pull/138) as
  `c40b82486a85867742a2f8fbfb516057b4130db5`. Implementation used historical
  `agent/p05-t9` / `.worktrees/p05-t9`; after review it rebased onto the
  independently approved pip prerequisite [#140](https://github.com/likefudan/ainvest/pull/140),
  squash `a6bcf1a711e95abfac7e605d5ccb9a6f395ae4cc`, and squash-merged in
  [#139](https://github.com/likefudan/ainvest/pull/139) as
  `67821569ba90884c522e8fb7efc341e24f597a3e`.
- **Claim review evidence:** [first review](https://github.com/likefudan/ainvest/pull/138#issuecomment-5358099207),
  [remediation](https://github.com/likefudan/ainvest/pull/138#issuecomment-5358262728),
  and [approval](https://github.com/likefudan/ainvest/pull/138#issuecomment-5358305214)
  bind the final claim contract to its approved exact head before squash.
- **Implementation review and rebase evidence:** [first functionality,
  security, and readability review](https://github.com/likefudan/ainvest/pull/139#issuecomment-5359059326),
  [first remediation](https://github.com/likefudan/ainvest/pull/139#issuecomment-5362069806),
  [second review](https://github.com/likefudan/ainvest/pull/139#issuecomment-5362168823),
  [second remediation](https://github.com/likefudan/ainvest/pull/139#issuecomment-5402855806),
  and [third-round approval](https://github.com/likefudan/ainvest/pull/139#issuecomment-5402886436)
  closed the implementation findings. The separate pip security prerequisite
  received [independent approval](https://github.com/likefudan/ainvest/pull/140#issuecomment-5402956106)
  and squash-merged before the implementation branch was
  [rebased and reverified](https://github.com/likefudan/ainvest/pull/139#issuecomment-5402994569).
  The [final post-rebase independent approval](https://github.com/likefudan/ainvest/pull/139#issuecomment-5403012527)
  verified range-diff equivalence, exact base/head, required green CI, and no
  P05-T9 semantic drift.
- **Post-rebase verification:** the canonical focused suite passed 302 tests
  plus one explicit optional-runtime skip; the locked PTB/broker focused suite
  passed 303. Unit passed 1,523 plus one skip, contract 203, integration 53,
  and combined verify 1,779 plus one skip, with 87.30% coverage. Ruff/format,
  mypy across 259 source files, lock-check, dependency audit, `git diff
...
- **Dependency history:** merged `P06-T2` Part 1, `P05-T4`, and `P05-T5`
  ...
  representative reads are verified; production Bot values and validation
  remain owner-assisted and pending under proposed `DEC-010`. Owner-assisted
...
- **Pinned history behavior:** `1d`, `5d`, `1m`, `3m`, and `1y` keep fixed
  rolling UTC durations of 24, 120, 720, 2,160, and 8,760 hours. The claimed
  maintenance changes only their intervals to `30minute`, `day`, `week`,
  `week`, and `month`; every request still uses `regular` bounds and `split`
  adjustment. No exchange-calendar dependency or trade-grade session claim is
  allowed.
...
##### Coordination claim: owner-validation follow-ups

- **Claim branch/worktree/base:** `agent/real-validation-followups-claim` /
  `.worktrees/real-validation-followups-claim` from immutable
  `7ef463f503612a3ef3bbc8623b47002a92ebc6e2`. This PR edits only
  `IMPLEMENTATION_TODO.md` and `docs/tasks/status.md`; it implements neither
  follow-up.
- **Sanitized owner evidence (2026-08-26):** `rh-mcp v0.3.3` authentication is
...
- **Completed parallel implementation / serial integration:** the claim
  squash-merged as `fa5e30f356a4082eb7a8b88c7d90ec7b3cbd68b1`.
  P05-T11 was independently reviewed and squash-merged first as
  `1a724e2861dc811c6c29717f6157c073bcc516f8`; P05-T9 history sizing then
  rebased without conflict, reverified, received independent approval, and
  squash-merged as `6815e7e03a3f2609e70d1e5b7281ff630c38e1be`.
  This serial integration is complete; no implementation branch remains
  claimed by this envelope.
...
- **Authority and exact behavior:** `IMPLEMENTATION_TODO.md` P05-T9's history
  ...
  call, one full render, at most one send, and the 3,500-code-point bound.
  Change only intervals to `30minute`, `day`, `week`, `week`, `month` for
  `1d`, `5d`, `1m`, `3m`, `1y`. Never truncate/split/paginate or make a second
  read; retain `result_too_large` for a deliberately oversized envelope.
- **Allowed implementation paths:**
  `src/ainvest/orchestrator/telegram_queries.py`;
  `tests/unit/orchestrator/test_telegram_queries.py`;
  `tests/integration/orchestrator/test_telegram_queries.py` only if the public
  handler boundary cannot be proved in unit scope; and
  `docs/telegram-read-queries.md`. Dependencies, lock, gateway/read client,
  models/mappers/display, configuration/account handling, Telegram transport,
  schemas, database, and every other path are read-only.
- **Required proof:** realistic handler tests for all five windows must each
  keep the complete normalized success reply at or below 3,500 code points and
  prove exactly one display/gateway call and one send. Retain a >3,500 fixture
...
- **Implementation evidence:** implementation code, tests, and user-facing
  ...
  source files. Immutable-head Verify, Secret scan, Dependency audit, SAST, and
  CodeQL were green; independent review approved exact head `ac39ea35` with no
  P0-P2 finding before squash merge as
  `6815e7e03a3f2609e70d1e5b7281ff630c38e1be`.

##### Execution envelope: P05-T11 read-account binding

- **Status/owner:** `merged` / `completed`; implementation owner
  `implement_account_provision`.
- **Implementation branch/worktree/base:** historical implementation branch
  `agent/p05-t11-account-provisioning` / worktree
  `.worktrees/p05-t11-account-provisioning`, created from exact merged-claim
  base `fa5e30f356a4082eb7a8b88c7d90ec7b3cbd68b1`. Independent review and
  remediation completed before squash merge as
  `1a724e2861dc811c6c29717f6157c073bcc516f8`.
- **Authority:** the complete P05-T11 card in `IMPLEMENTATION_TODO.md`. Add only
  dedicated `ainvest-robinhood-account provision|validate|disable` in
  `ainvest.orchestrator.robinhood_account_provisioning`. It uses one pinned
  named `read_accounts` call, requires exactly one active
  `agentic_allowed=true` record, never prints the result/value, and installs
  only the exact file-secret. P05-T10's four commands stay unchanged and no
  `approval -> execution` import is introduced.
- **Allowed implementation paths:** new
  `src/ainvest/orchestrator/robinhood_account_provisioning.py`; optional narrow
  `src/ainvest/orchestrator/__init__.py`; `pyproject.toml` only for the one
...
- **Security and verification:** sole production source is the explicit
  ...
  never writes, rotation is explicit disable then provision, and disable is
  exact-target/idempotent after external poller stop plus maintenance-lease
  coordination. Tests use synthetic values and fake named gateways and prove
  sanitized output, one call, lifecycle closure, atomic/no-follow handling,
