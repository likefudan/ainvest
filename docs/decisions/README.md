...
| `DEC-003` | Prefer contract-tested, officially supported Robinhood MCP read capabilities behind the default-deny Non-Trading Gateway, whose reviewed manifest allows 35 read capabilities and 11 non-trading mutations and permanently denies all 8 trading capabilities. Ainvest exposes only its existing 10-operation named read projection. Robinhood MCP is the sole live quote source; there is no live fallback to Alpaca, yfinance, or another provider. | `accepted` | Product owner | Accepted 2026-07-26 | If a required MCP capability is unavailable, stale, incomplete, internally inconsistent, or schema-incompatible, reject trading. Optional sources remain development/offline only. | Phase 04 (`P04-T0`–`P04-T5`), Phase 06 (`P06-T0`–`P06-T3`), Phase 07 (`P07-T1`) |
...
| `DEC-010` | Create separate staging and production Telegram Bots and provide, for each environment, the positive numeric expected Bot ID plus bound allowed `(user_id, private_chat_id)` recipient records. Bot tokens stay only in secret storage. | `proposed` | Product/account owner | Before owner-assisted real provisioning/validation in `P05-T10` and before Telegram integration is enabled | Telegram integration and approval consumption are disabled. No real token or ID is recorded in Git. Tests and examples use disabled synthetic IDs and fake/empty token values only. | Phase 05 (`P05-T1`, `P05-T4`, `P05-T5`, `P05-T8`–`P05-T10`) |
...
| `DEC-017` | Authorize the dedicated Robinhood Agentic Account and set its live budget. | `deferred_until_live` | Product/account owner | Before any real-order exercise in `P07-T6` | No Robinhood write credentials or capability are provisioned; live trading remains disabled. | Phase 06 (`P06-T0`–`P06-T3` non-trading authorization as applicable), Phase 07 (`P07-T0`, `P07-T1`, `P07-T6`) |
