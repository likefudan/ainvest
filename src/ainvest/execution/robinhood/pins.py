...
Every value here is transcribed from **one** authority: the
``Recorded external dependency pin: likefudan/rh-mcp v0.2.0`` subsection of
`docs/tasks/status.md`. Nothing here may be re-derived from `rh-mcp`'s
``CHANGELOG.md``: its ``[0.1.0]`` and ``[0.2.0]`` entries both print
``sha256:49b7218…`` beside manifest version ``2026.08.03.1``, and those two
values do not belong together — ``49b7218…`` is the digest of manifest
``2026.08.05``, which `rh-mcp` ``main`` ships and ``v0.2.0`` does not. A
consumer that pinned the ``v0.2.0`` artifact and took that digest would fail
readiness at every startup, so :data:`REJECTED_CHANGELOG_MANIFEST_DIGEST`
records the wrong value explicitly and a contract test asserts it is never
accepted.
...
full-manifest digest from the committed ``v0.2.0`` manifest with an
...
PINNED_RELEASE_TAG: Final = "v0.2.0"

#: ``Version:`` recorded in the wheel's ``METADATA``.
PINNED_PACKAGE_VERSION: Final = "0.2.0"
...
PINNED_SOURCE_COMMIT: Final = "46128a623c87f954c18d037870e4ac36b9e61e13"

PINNED_WHEEL_FILENAME: Final = "rh_mcp-0.2.0-py3-none-any.whl"
PINNED_WHEEL_SHA256: Final = "45bdfa7ef191a5dca834ddf52249fd92cfce0cf33456ec26839bdc8024e657b9"
PINNED_WHEEL_SIZE_BYTES: Final = 195_380

PINNED_SDIST_FILENAME: Final = "rh_mcp-0.2.0.tar.gz"
PINNED_SDIST_SHA256: Final = "da1d2231fd7be4129e035879eec4965727b968496c382bdaaa6f663bec11842c"
PINNED_SDIST_SIZE_BYTES: Final = 381_656
...
PINNED_MANIFEST_VERSION: Final = "2026.08.03.1"
...
PINNED_PROVIDER_SURFACE_DIGEST: Final = (
    "sha256:a7b23c7bcf5b563a35f21331ece72a90d5df1be8149cd7b90303565d2f1b1e32"
)
...
# ---------------------------------------------------------------------------
# The reviewed manifest's three dispositions (rule 32: 34 / 11 / 8)
# ---------------------------------------------------------------------------

#: The 34 ``allowed`` capabilities with ``mutates=false``.
...
class ReadCapability(StrEnum):
    ...
    the manifest's 34 ``mutates=false`` entries with what ainvest actually
